...
Destroy the mountains before your starship collides with one of them. For that,
...
highest mountain by outputting its index (from 0 to 7).
...
Don’t forget to run the tests by launching them from the _Test cases_ window.
The tests provided and the validators used to calculate your score are slightly
different to avoid hard-coded solutions.
...
-   **8 lines**: one integer `mountainH` per line. Each represents the height
    of one mountain given in the order of their index (from 0 to 7)
